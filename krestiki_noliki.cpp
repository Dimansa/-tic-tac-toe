...
int vvod(int i,int *b,int *c)
{
	...
	{
		cout << "¬аш ход, введите координаты (две цифры от 1 до 3)" << endl;
		if (cout.fail() == true)
			return 3;
		while (!(cin >> k1[i] >> k2[i])) 
		{
			cout << "¬ведено некорректное значение, ¬ведите цифры от 1 до 3" << endl;
			if (cout.fail() == true) 
				return 4;
			cin.clear();
			cin.ignore(32767, '\n');
		...
		{
			cout << "Ёта клетка уже зан€та или не существует, поставте крестик в другую клетку" << endl;
			if (cout.fail() == true) 
				return 6;
			q=vvod(i,b,c);
			if (q != 0)
...
int main() 
{
	...
	if(setlocale(LC_ALL, "Russian")==NULL)
		return 7;
	cout << "ƒобро пожаловать" << endl << "1 1 это левый верхний угол, 1 2 это середина вверху, а 3 3 это правый нижний угол" << endl;
	if (cout.fail() == true)
		q = 1;
	for (int i = 0; i < 3; i++)
		for (int j = 0; j < 3; j++)
